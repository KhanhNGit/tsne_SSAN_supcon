...
class Normalization_ImageNet(object):
    """
        normalize into [0, 1]
        image = image/255
    """
...
def transformer_train():
    return transforms.Compose([Contrast_and_Brightness(), RandomHorizontalFlip(), ToTensor(), Cutout(), Normalization()])


def transformer_train_pure():
    return transforms.Compose([RandomHorizontalFlip(), ToTensor(), Normalization()])


def transformer_train_ImageNet():
    return transforms.Compose([RandomHorizontalFlip(), ToTensor(), Normalization_ImageNet()])
